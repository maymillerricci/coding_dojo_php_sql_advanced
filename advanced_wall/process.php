<?php
	...
	if(isset($_POST['action']) && $_POST['action'] == "login")
	{
		...
		$login_password = md5(mysql_real_escape_string($_POST['login_password']));
	...
	if(isset($_POST['action']) && $_POST['action'] == "post_message")
	{
		...
		else
		{
			...
			$query = "INSERT INTO messages (user_id, message, created_at) VALUES ('{$_SESSION['user_info']['id']}', '{$message}', NOW())";
			mysql_query($query);
			header("location: wall.php");
		}
	}
	...
	if(isset($_POST['action']) && $_POST['action'] == "post_comment")
	{
		...
		else
		{
			...
			$query = "INSERT INTO comments (user_id, message_id, comment, created_at) VALUES ('{$_SESSION['user_info']['id']}', '{$_POST['message_id']}', '{$comment}', NOW())";
			mysql_query($query);
			header("location: wall.php");
		}
	}
